...
- **ops-dashboard** — removed the redundant `env` block from `.mcp.json`. `${PUBLIC_STORE_DOMAIN}` / `${SHOPIFY_ADMIN_API_TOKEN}` are expanded from the **shell** environment, not from `.env`, so on a normal shell they resolved to nothing and every session logged `Missing environment variables: PUBLIC_STORE_DOMAIN, SHOPIFY_ADMIN_API_TOKEN`. The server already self-loads `.env` via its own `loadDotEnv()`, so the block was never needed.
- **ops-dashboard** — the local `.env` defined `SHOPIFY_ADMIN_API_TOKEN` **twice** with two different values. `loadDotEnv()` is first-wins (`if (!process.env[key])`), so the server was picking up the earlier entry — a 32-char hex string that is an app API secret key, not an access token, and which the Admin API rejects with **401**. The real `shpat_…` token was the shadowed second entry (verified **200** against `/admin/api/2025-07/shop.json`). The duplicate is removed locally; `.env` is gitignored, so this is a machine-local fix with no tracked diff. Handshake success was masking it — the server connected fine and only failed on actual Admin API calls.
