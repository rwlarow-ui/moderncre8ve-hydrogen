...
import type { PageImage } from "~/page-builder";
import { createSchema, type SectionComponentProps } from "~/page-builder";

interface CustomerGalleryItemProps extends SectionComponentProps {
  image?: PageImage;
  caption: string;
  customerName: string;
  productUrl?: string;
...
const CustomerGalleryItem = forwardRef<
  HTMLDivElement,
  CustomerGalleryItemProps
>((props, ref) => {
  const {
    image,
  ...
  return (
    ...
    >
      ...
      >
        <p className="font-medium font-sans text-sm text-white">{caption}</p>
        {customerName && (
          <p className="font-serif text-white/80 text-xs">{customerName}</p>
        )}
      </div>
